...
public:
   ...
   bool locateRanges()
   {
      ...
      rowD = 2; //getRowFromName("datamarker_hourly") + 1;
      colD = 0; // getColFromName("datamarker_hourly") - 1;
   ...
   void readGSSheet()
   {
      ...
      if (useGSData && GSFileName != "") // only if we actually selected a file, and we're using the GS data files in the first place
      {
        ...
        yearCount = 0; // how many unique years have we encountered?
         long curYear = 0;
         long readYear = 0;
    ...
    void riley_gs_in(S4 modelobj){
      memset(GSCells, 0, sizeof(GSCells));
      if (useGSData){
          // yoink df col to NumericVector
          // yoink NumericVector into gsCells
          S4 riley_p = modelobj.slot("Parameters");
          Rcpp::RObject gsrdid = riley_p.slot("gs_limits");
          Rcpp::DataFrame gsr_d_1 = Rcpp::DataFrame(gsrdid.get__());


          NumericVector gsriley_yoink0 = gsr_d_1[0];
          NumericVector gsriley_yoink1 = gsr_d_1[1];
          NumericVector gsriley_yoink2 = gsr_d_1[2];
          // fuck i am stupid
          // this is the dumbest possible way to do this
          for(int gsrirow = 2; gsrirow < riley_year_count + 2; gsrirow++){
              GSCells[gsrirow][0+1] = gsriley_yoink0[gsrirow-2];
              GSCells[gsrirow][1+1] = gsriley_yoink1[gsrirow-2];
              GSCells[gsrirow][2+1] = gsriley_yoink2[gsrirow-2];
          }
          yearCount = riley_year_count;
        }}

        void riley_data_in(S4 modelobj){
          memset(dataCells, 0, sizeof(dataCells));
              // yoink df col to NumericVector
              // yoink NumericVector into dataCells

              Rcpp::RObject rdid = modelobj.slot("data");
              Rcpp::DataFrame r_d_1 = Rcpp::DataFrame(rdid.get__());




              NumericVector riley_yoink0 = r_d_1[0];

              NumericVector riley_yoink1 = r_d_1[1];
              NumericVector riley_yoink2 = r_d_1[2];
              NumericVector riley_yoink3 = r_d_1[3];
              NumericVector riley_yoink4 = r_d_1[4];
              NumericVector riley_yoink5 = r_d_1[5];
              NumericVector riley_yoink6 = r_d_1[6];
              NumericVector riley_yoink7 = r_d_1[7];
              NumericVector riley_yoink8 = r_d_1[8];

              // fuck i am stupid
              // this is the dumbest possible way to do this
              for(int rirow = 2; rirow < riley_nr_chillin + 2; rirow++){

                  dataCells[rirow][0+1] = riley_yoink0[rirow-2];
                  dataCells[rirow][1+1] = riley_yoink1[rirow-2];
                  dataCells[rirow][2+1] = riley_yoink2[rirow-2];
                  dataCells[rirow][3+1] = riley_yoink3[rirow-2];
                  dataCells[rirow][4+1] = riley_yoink4[rirow-2];
                  dataCells[rirow][5+1] = riley_yoink5[rirow-2];
                  dataCells[rirow][6+1] = riley_yoink6[rirow-2];
                  dataCells[rirow][7+1] = riley_yoink7[rirow-2];
                  dataCells[rirow][8+1] = riley_yoink8[rirow-2];
   ...
   void readDataSheet()
   {
    ...
    void readin(S4 modelobj) //'inputs and calculates all parameters at the start
       {
          ...
          lightcomp = riley_p_photos.slot("light_comp");//getValueFromNameDbl("i_lightComp"); //Cells(14, 21) //light compensation point in ppfd
          ...
          sthresh = 1.0001; //acceptable error for e integral for xylem
           ...
           br = ri_w_root[0];//getValueFromNameDbl("i_br"); //getInputValue(wbRange, 3, 1) //Cells(4, 2) // = br //weibull b for each root element
           cr = ri_w_root[1];//getValueFromNameDbl("i_cr"); //getInputValue(wbRange, 4, 1) //Cells(5, 2) // = cr //weibull c for each root element
           bs = ri_w_stem[0];//getValueFromNameDbl("i_bs"); //getInputValue(wbRange, 3, 2) //Cells(4, 3) // = bs //weibull parameters for stem
           cs = ri_w_stem[1];//getValueFromNameDbl("i_cs"); //getInputValue(wbRange, 4, 2) //Cells(5, 3) //= cs
                                            //bl = bs: cl = cs //stem and leaf curves equal
           bl = ri_w_leaf[0];//getValueFromNameDbl("i_bl"); //getInputValue(wbRange, 3, 2) //Cells(4, 4) // = bl //weibull parameters for leaf
           cl = ri_w_leaf[1];//getValueFromNameDbl("i_cl"); //getInputValue(wbRange, 4, 2) //Cells(5, 4) // = cl

           lat = riley_p_stand.slot("latitude"); //getValueFromNameDbl("i_latitude"); //getInputValue(standRange, 1, 1) //Cells(5, 18) //latitude in degree fraction north
          lat = pi / 180 * lat; //converted degrees to radians
          longitude = riley_p_stand.slot("longitude");//getValueFromNameDbl("i_longitude"); //getInputValue(standRange, 2, 1) //Cells(6, 18) //longitude in degree fraction west
           tau = riley_p_atmos.slot("atm_trans");//getValueFromNameDbl("i_atmTrans"); //Cells(4, 18) //atmospheric transmittance from weather data
           tsncorr = riley_p_stand.slot("solar_noon_corr");//getValueFromNameDbl("i_solarNoon"); //Cells(14, 18) //solar noon correction from weather data
           slope = riley_p_stand.slot("slope_inc");//getValueFromNameDbl("i_slopeI"); //Cells(7, 18) //slope inclination, degrees from horizontal
           slopeasp = riley_p_stand.slot("slope_aspect");//getValueFromNameDbl("i_slopeA"); //Cells(8, 18) //slope aspect, counterclockwise degrees from south
           lai = riley_p_stand.slot("canopy_lai");//getValueFromNameDbl("i_leafAngleIndex"); //Cells(10, 18) //canopy lai
           xang = riley_p_stand.slot("leaf_angle_p");//getValueFromNameDbl("i_leafAngleParam"); //Cells(9, 18) //leaf angle parameter, CN 15.4
          ...
          gmax = 1000000; //Cells(2, 13) //maximum G, wet soil, vpd = 0, in kg m - 2 hr - 1 basal area
           laperba = riley_p_plant.slot("la_to_ba");//getValueFromNameDbl("i_leafPerBasal"); //Cells(1, 13) //initial leaf area per basal area, m2 m - 2
          ...
          patm = 101.325 * pow((1 - 0.0065 * alt / (288.15 + 0.0065 * alt)), 5.257); //atmospheric pressure, T = 15 C, average sealevel patm; approximation
           ...
           leafpercent = ri_w_leaf[2];//getValueFromNameDbl("i_leafPercRes"); //Cells(6, 4) //saturated % of tree R in leaves
          ...
          //Cells(1, 9) = ksatp
           ...
           ksatp = riley_p_plant.slot("plant_kmax");//getValueFromNameDbl("i_kmaxTree"); //Cells(1, 9) //kmax of tree in kg hr - 1m - 2MPa - 1 per basal area
          ...
          ksats = 1.0 / ((stempercent / 100.0) * rsatp); //kmax of stem system
                                                         ...
                                                         //Cells(3, 7) = dbh
           height = riley_p_plant.slot("height");//getValueFromNameDbl("i_height"); //Cells(3, 9) //average tree height in m
          ...
          einc = ksatp / 500.0; //e increment in kg hr-1 m-2 basal area for composite curve
           ...
           pinc = riley_p_soil.slot("p_inc");//getValueFromNameDbl("i_pinc");
          if (pinc <= 0.0)
          ...
          for (k = 1; k <= layers; k++) //To layers //read in soil
          {
             ...
             //a[k] = atof(paramCells[rowLR + k][colLR + 3].c_str());
              ...
              a[k] = ri_vg_a[k - 1];//atof(paramCells[rowLR + k][colLR + 3].c_str()); //van genuchten alpha
              n[k] = ri_vg_n[k - 1];//atof(paramCells[rowLR + k][colLR + 4].c_str()); //lSheet.Cells(rowLR + k, colLR + 4) //vg n
              kkmax[k] = ri_ksat[k - 1];//atof(paramCells[rowLR + k][colLR + 5].c_str());//saturated conductivity of soil in kg hr-1MPa-1 m-1
    ...
    // FUCKUP LOWER BOUND
          ...
          for (k = 1; k <= layers; k++)//k = 1 To layers
          {
           ...
           //refilling = riley_p_opt.slot("refilling");//getValueFromNameStr("i_refilling"); //Cells(4, 12) //"y/n"
                                                          //A-ci curve parameters...A = aaa(1-bbb^ci)
                                                          //par = Cells(3, 21)//now read in each time step
           qmax = riley_p_photos.slot("quantum_yield_e_t");//getValueFromNameDbl("i_qMax"); //Cells(3, 21) //quantum yield of electron transport, moles e per mols photons
           ca = riley_p_photos.slot("atm_co2_ppm");//getValueFromNameDbl("i_co2AmbPPM"); //Cells(2, 21) //ambient co2 in ppm
          ca = ca * 0.000001; //ambient co2 in moles per mole
           ...
           vmax25 = riley_p_photos.slot("vmax");//getValueFromNameDbl("i_vmax25"); //Cells(4, 21) //vmax at 25C
                                                    //vmax25initial = vmax25
           jmax25 = riley_p_photos.slot("jmax");//getValueFromNameDbl("i_jmax25"); //Cells(5, 21) //jmax at 25C
                                                    //rday25 = Cells(8, 21) //day respiration at 25C, = 0.015vmax cf.collatz et al.
           kc25 = riley_p_photos.slot("kc");//getValueFromNameDbl("i_kc25"); //Cells(6, 21) //m - m constant for CO2 in mole fraction at 25C
           ko25 = riley_p_photos.slot("ko");//getValueFromNameDbl("i_ko25"); //Cells(7, 21) //m - m constant for O2 in mole fraction at 25C
           comp25 = riley_p_photos.slot("comp_point");//getValueFromNameDbl("i_comp25"); //Cells(8, 21) //photorespiratory compensation point in mole fraction at 25C
           thetac = riley_p_photos.slot("theta");//getValueFromNameDbl("i_thetaC"); //Cells(9, 21) //shape factor for A - ci colimitation, 0.98
           ...
           leafwidth = riley_p_plant.slot("leaf_width");//getValueFromNameDbl("i_leafWidth") * 0.72; //Cells(2, 18) * 0.72 //leaf width x factor = characteristic dimension(campbell and norman)
           leafwidth = leafwidth*0.72;
           emiss = riley_p_atmos.slot("sky_emiss");//getValueFromNameDbl("i_emiss"); //Cells(3, 18) //long wave emissivity
           havmax = riley_p_photos.slot("ha_vmax");//getValueFromNameDbl("i_havmax"); //Cells(10, 21) //these are all temp - dependency parameters from Leunig 2002
          hdvmax = riley_p_photos.slot("hd_vmax");//getValueFromNameDbl("i_hdvmax"); //Cells(11, 21)
          svvmax = riley_p_photos.slot("sv_vmax");//getValueFromNameDbl("i_svvmax"); //Cells(12, 21)
          hajmax = riley_p_photos.slot("ha_jmax");//getValueFromNameDbl("i_hajmax"); //Cells(10, 23)
          hdjmax = riley_p_photos.slot("hd_jmax");//getValueFromNameDbl("i_hdjmax"); //Cells(11, 23)
          svjmax = riley_p_photos.slot("sv_jmax");//getValueFromNameDbl("i_svjmax"); //Cells(12, 23)
           lightcurv = riley_p_photos.slot("curv_light_resp");//getValueFromNameDbl("i_lightCurv"); //Cells(13, 21)
           baperga = riley_p_stand.slot("ba_to_ga");//getValueFromNameDbl("i_baperga") * 0.0001; //Cells(3, 27) * 0.0001 //basal area per ground area converted from m2 / Ha to m2 / m2
           baperga = baperga * 0.0001;
           ffc = riley_p_soil.slot("init_p_field_cap");//getValueFromNameDbl("i_fieldCapPercInit") / 100.0; //Cells(2, 27) / 100 //fraction of field capacity for starting the season
           ffc = ffc / 100.0;
          fieldcapfrac = riley_p_soil.slot("field_cap_frac"); //getValueFromNameDbl("i_fieldCapFrac"); //Cells(1, 27) //fraction that field capacity is of saturation(minus residual)
           ...
           //Rcout << "after readin sevap is currently: " << sevap << " \n";
          ...
          //sevap = getValueFromNameStr("i_soilEvapEnable"); //Cells(9, 27) //turns off / on soil evaporation routine
           soilabssol = riley_p_soil.slot("soil_abs");//getValueFromNameDbl("i_soilAbsSol"); //Cells(11, 18) //absorptivity of soil surface for solar
          ...
          zdispl = 6.5 * rough; //soil d, eqn 14.9, using d = 6.5 Zm, eq 5.2,5.3
           ...
           xh = xh * 100; // riley:: misunderstood, reverting to * 100 because it was right all along.  Morning brain
          ...
          {
             if (riley_p_opt.slot("use_gs_data")) {   //getValueFromNameStr("i_useGSDataStress") == "y"
                useGSData = true;
             }
             else {
                useGSData = false;
   ...
   short cleanModelVars()
   {
      ...
      soiltemp = 0;
      night = "";
      //Rcout << "before cleanvars sevap is currently: " << sevap << " \n";
      sevap = "";
      //Rcout << "after cleanvars sevap is currently: " << sevap << " \n";
      pet = "";
      rainsim = "";
      nightBool = 0;
      tau = 0;
   ...
   fortyMarker:
      ...
      if (ground == "y")
         deepflow(); //'gets groundwater flow into bottom layer in m3/m2
                     //'} //End if// //'pet <> y or n
      if(dd % 100 == 0){
        //Rcout << "in modeltimestepiter sevap is currently: " << sevap << " \n";
      }
   ...
   void modelProgramNewYear(S4 modelobj)
   {
      ...
      readin(modelobj); // get all the parameters again
  ...
  /*
        for(int coln=1; coln < ncol_sumhead + 1; coln++){
          ...
          NumericVector ozzy {};
          for(int rown=2; rown < riley_year_count + 3; rown++){
            ozzy.push_back(dataCells[rown][coln]);
          }
         ...
         for(int i=3; i < 3+yearCount; i++){
           NumericVector ozzy(finalOutCells[i]+1,finalOutCells[i]+34);
...
long ModelProgram::modelProgramMain(S4 modelobj) //program starts here
{
   ...
   cleanModelVars();
   initModelVars();
   readin(modelobj); //get all global parameters
   ...
   //readDataSheet(path9);
   riley_data_in(modelobj);
   riley_gs_in(modelobj);
   //readGSSheet(path9);
...
int runit2(S4 modelobj, NumericVector jmax_vary = 0, NumericVector vmax_vary = 0, NumericVector lai_vary = 0, List swc_vary = 0, NumericVector dep_vary = 0, NumericVector rad_vary = 0, int read_n_layers = 5, std::string nametag = "spemo", int riyc = 0, int rinr = 0)
{
    ...
    // so here i am in the office at 10pm on a tuesday.  club goin up
    // but it's not like i have anything fun or exciting to do at home so whatever
    // fuck richlandwashington
    // i have made my own bed and now i must lie in it
    // name tag time
    globalnametag = nametag;

    // yc time
    riley_year_count = riyc;

    // nr time
    riley_nr_chillin = rinr;
    ...
    //Rcpp::RObject ri_df_init_data = modelobj.slot("data");
    //Rcpp::DataFrame r_d_1 = Rcpp::DataFrame(ri_df_init_data.get__());
