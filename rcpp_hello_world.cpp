...
public:
   ...
   bool locateRanges()
   {
      ...
      std::string paramFileName = "/Users/rileyleff/Desktop/eztest/parameters.csv";
      std::string nameTableFileName = "/Users/rileyleff/Desktop/eztest/nametable.csv";

      std::cout << "Riley_Test298 Confirmed" << std::endl;
      ...
      std::cout << "Read site and scenario names (This is all random junk that Riley needs to remove from the codebase): " << name_region << " | " << name_site << " | " << name_species << " | " << name_scen << " | " << name_model << std::endl;
   ...
   void readGSSheet()
    ...
    void old_readGSSheet(){
      ...
      {
         GSFileName = "/Users/rileyleff/Desktop/eztest/seasonlimits.csv";
      }
   ...
   void readDataSheet(S4 modelobj)
   {
      // try to find a header file
      bool foundHeaderFile = false;
      std::string headerFileName = "/Users/rileyleff/Desktop/eztest/dataheader.csv";
      ...
      headerFileName = "/Users/rileyleff/Desktop/eztest/sumheader.csv";
      ...
      std::string dataFileName = "/Users/rileyleff/Desktop/eztest/dataset.csv";

      if (stage_ID == STAGE_ID_NONE)
      {
         dataFileName = "/Users/rileyleff/Desktop/eztest/dataset.csv";
      }
...
// FUCKUP LOWER BOUND
      ...
      for (k = 1; k <= layers; k++)//k = 1 To layers
      {
       ...
       refill_choice = riley_p_opt.slot("refilling");
       if(refill_choice){
           refilling = "y";
       } else{
           refilling = "n";
   ...
   void readGrowSeasonData(S4 modelobj)
    {
        S4 ri_par = modelobj.slot("Parameters");
        
        Rcpp::RObject ri_gs_init = ri_par.slot("gs_limits");
        Rcpp::DataFrame ri_gs_x = Rcpp::DataFrame(ri_gs_init.get__());
             
        NumericVector ri_gs_y = ri_gs_x[0];
        NumericVector ri_gs_s = ri_gs_x[1];
        NumericVector ri_gs_e = ri_gs_x[2];
        
        int count = 0 ;
        
        do{
          gs_ar_years[count] = ri_gs_y[count];
          gs_ar_starts[count] = ri_gs_s[count];
          gs_ar_ends[count] = ri_gs_e[count];
            count = count + 1;
        } while (!ri_gs_y[count] == 0);
    }
    
    void old_readGrowSeasonData(){
...
// aw4toili8  <-- my cat wrote this by walking across my keyboard
// good job Otto Von Beansmark
// world's #1 dev cat
   ...
   void readIterationSettings()
   {
       ...
       iter_gwInc = 1.2;//getValueFromNameDbl("i_iter_gwInc");
       iter_gwStart = 12;//getValueFromNameDbl("i_iter_gwStart");
       iter_gwEnd = 4;//getValueFromNameDbl("i_iter_gwEnd");

         iter_ffcEnable = false;

       iter_ffcInc = 0.2;//getValueFromNameDbl("i_iter_ffcInc");
       iter_ffcStart = 0.1;//getValueFromNameDbl("i_iter_ffcStart");
       iter_ffcEnd = 1;//getValueFromNameDbl("i_iter_ffcEnd");
       
         iter_bagaEnable = false;

       iter_bagaInc = 5;//getValueFromNameDbl("i_iter_bagaInc");
       iter_bagaStart = 5;//getValueFromNameDbl("i_iter_bagaStart");
       iter_bagaEnd = 150;//getValueFromNameDbl("i_iter_bagaEnd");
       iter_bagaRef = 2;//getValueFromNameDbl("i_iter_bagaRef");
       iter_bagaCutoff = 0.9;//getValueFromNameDbl("i_iter_bagaCutoff");
...
long ModelProgram::modelProgramMain(S4 modelobj) //program starts here
{
   ...
   readDataSheet(S4 modelobj);
   old_readGSSheet();
   old_readGrowSeasonData(); //hnt todo cleanup - should just put this in readin?
   ...
   //saveOutputSheet("/Users/rileyleff/Desktop/" + stageNames[stage_ID] + "_OUTPUT_timesteps", "timesteps");
   //saveOutputSheet("/Users/rileyleff/Desktop/" + stageNames[stage_ID] + "_OUTPUT_summary", "summary");
